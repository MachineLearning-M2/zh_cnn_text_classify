...
参考[IMPLEMENTING A CNN FOR TEXT CLASSIFICATION IN TENSORFLOW](http://www.wildml.com/2015/12/implementing-a-cnn-for-text-classification-in-tensorflow/)
...
run 
```tensorboard --logdir /{PATH_TO_CODE}/runs/{TIME_DIR}/summaries/```
 to view summaries in web view

### 测试、分类
run 
```python eval.py --checkpoint_dir /{PATH_TO_CODE/runs/{TIME_DIR}/checkpoints}```<br/>
